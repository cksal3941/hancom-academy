...
한컴아카데미 웹사이트를 React 기반 SPA로 리뉴얼한 프로젝트입니다.  
기존 사이트는 반응형이 구현되어 있지 않았으며, 이번 리뉴얼을 통해 데스크탑·태블릿·모바일 전 구간 반응형을 새롭게 구현했습니다.  
메인 슬라이드, 반응형 헤더, 모바일 메뉴, 교육 과정 소개, 공지/뉴스 게시판, 위치 안내, 로그인/회원가입 기능을 포함합니다.

| 항목 | 내용 |
| --- | --- |
| 프로젝트명 | 한컴아카데미 Website |
...
| 이미지 업로드·라이트박스 기능 추가 | 2026.07.01 ~ 2026.07.02 |
...
- Cloudinary 이미지 업로드 및 라이트박스 뷰어 구현
- Vercel 배포 및 환경변수 구성
...
한컴아카데미 웹사이트는 교육기관의 브랜드 정보, 교육 과정, 설명회, 위치 안내를 한 화면 흐름으로 제공하는 웹사이트입니다.  
...
- ✅ 게시글 이미지 라이트박스 (클릭 확대 / 스크롤 줌 / 드래그 이동)
- ✅ 수상실적 스크롤 타임라인
...
| 게시판 CRUD | Firestore 연동, 이미지 Cloudinary 업로드, 정적 데이터 fallback 처리 |
| 이미지 라이트박스 | 클릭 확대(1×→1.5×→2×→3× 순환), 스크롤 휠 줌, Pointer API 드래그 이동 |
...
| Cloudinary 업로드 400 오류 | 업로드 프리셋이 Signed 모드로 설정됨 | Cloudinary 대시보드에서 프리셋을 Unsigned 모드로 변경 |
| 라이트박스에서 이미지가 더 작게 표시 | 썸네일은 `width:100%`로 늘어나지만 라이트박스는 원본 크기로 표시 | `min-width: min(680px, 90vw)` 적용으로 최소 크기 보장 |
...
- 메인 슬라이드 이미지를 PNG → JPG로 교체해 용량을 절감했습니다.
...
│  │  ├─ cards/            # 카드 컴포넌트
│  │  ├─ common/           # SubPageHero, MobileMenu, NoticeImages
│  │  ├─ home/             # 홈 섹션 컴포넌트
│  │  └─ location/         # 지도 관련 컴포넌트
│  ├─ data/                # 화면 데이터
...
│  ├─ services/            # authService, noticeService, newsService, cloudinaryService
...
- Cloudinary Unsigned Upload를 연동해 별도 서버 없이 이미지를 저장하는 구조를 경험했습니다.
- Pointer API를 활용한 드래그·줌 인터랙션 구현 방법을 익혔습니다.
...
이번 프로젝트는 반응형이 전혀 구현되지 않았던 기존 사이트를 완전히 리뉴얼하는 작업이었습니다.  
단순한 랜딩 페이지가 아니라 실제 교육기관 웹사이트에 필요한 정보 구조와 사용자 흐름을 고려하며 제작했습니다.  
컴포넌트 재사용, 데이터 분리, 인증 기능, 반응형 메뉴, Firestore 게시판 연동, Cloudinary 이미지 업로드까지 프론트엔드 프로젝트의 전체 흐름을 경험할 수 있었습니다.

**좋았던 점:**
- 반응형을 처음부터 설계에 반영해 데스크탑·태블릿·모바일 전 구간에서 자연스러운 레이아웃을 구현했습니다.
- 화면 단위 컴포넌트를 분리해 유지보수성을 높였습니다.
- Firestore + Cloudinary 연동으로 서버 없이 실제 데이터 흐름을 경험했습니다.
- 메인 비주얼과 퀵메뉴를 통해 첫 화면의 사용성을 강화했습니다.

**아쉬운 점:**
- 일부 교육과정 서브페이지는 아직 준비 중 상태입니다.
- 이미지 최적화(WebP 전환)와 테스트 자동화는 추가 개선이 필요합니다.
...
```text
Copyright (c) 2026 한컴아카데미 Website
All rights reserved.
```
